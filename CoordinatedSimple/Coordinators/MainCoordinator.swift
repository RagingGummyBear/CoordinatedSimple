...
class MainCoordinator:NSObject, Coordinator {
    ...
    func start(){
        ...
        self.dataProvider.readAllUsers().done(on: DispatchQueue.main, flags: nil) { (result: [UserModel]) in
            print(result)
            _ = self.dataProvider.testPersistentData(users: result).catch({ (error:Error) in
                print(error)
        ...
        self.dataProvider.readUser(userID: 2).done { (result: UserModel) in
            print(result.name!)
        }.catch { (error: Error) in
            print(error)
        }
